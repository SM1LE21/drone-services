import { services_2 } from "../constants";
import styles, { layout } from "../style";
import { GetStartedButtonEstate } from "./Button";

const FeatureCard = ({ price, title, content, index }) => (
  <div className={`flex flex-row p-6 rounded-[20px] ${index !== services_2.length - 1 ? "mb-6" : "mb-0"} feature-card`}>
    <div className={`w-[64px] h-[64px] rounded-full ${styles.flexCenter} bg-dimBlue`}>
    <p className="text-secondary">{price} Є</p>
    </div>
    <div className="flex-1 flex flex-col ml-3">
      <h4 className="font-poppins font-semibold text-white text-[18px] leading-[23.4px] mb-1">
        {title}
      </h4>
      <p className="font-poppins font-normal text-dimWhite text-[16px] leading-[24px]">
        {content}
      </p>
    </div>
  </div>
);

const Services2 = () =>  (
  <section id="services" className={layout.section}>
    <div className={layout.sectionInfo}>
      <h2 className={styles.heading2}>
      Real Estate Photography
      </h2>
      <p className={`${styles.paragraph} max-w-[470px] mt-5`}>
      Set your real estate properties apart with stunning aerial imagery. Our certified drone pilots specialize in capturing the beauty and uniqueness of each property, providing potential buyers with a dynamic and attractive presentation that increases interest and engagement.
      </p>

      <GetStartedButtonEstate styles={`mt-10`} />
    </div>
    <div className={`${layout.sectionImg} flex-col`}>
      {services_2.map((service, index) => (
        <FeatureCard key={service.id} {...service} index={index} />
      ))}
    </div>
  </section>
);

export default Services2;
